...
const {RPC} = require('@nexellia/grpc-node');
...
//TODO
window.PWA_MODULES={};
window.PWA_MODULES["@nexellia/wallet-pwa"] = "N/A";
window.PWA_MODULES["@aspectron/flow-ux"] = "N/A";
window.PWA_MODULES["@nexellia/ux"] = "N/A";
window.PWA_MODULES["@nexellia/grpc-web"] = "N/A";
window.PWA_MODULES["@nexellia/wallet"] = "N/A";
window.PWA_MODULES["@nexellia/grpc"] = "N/A";
window.PWA_MODULES["@nexellia/core-lib"] = "N/A";
...
//			['Mosquitto','EDL-V10 EPL-V10','Copyright (c) 2007, Eclipse Foundation, Inc. and its licensors'],
		...
		<tab-content for="home">
			...
			<flow-expandable class="donation-addresses" no-info _icon="fal:donate">
				...
				<p is="i18n-p">
					If you wish to further the development of the nexellia ecosystem, we accept nexellia donations at the following addresses:
				</p>
				${
		...
		<tab-content for="settings">
			...
			<flow-form-control icon="fal:database">
				...
				<p slot="info" is="i18n-p">
					It will delete datadir (Data Folder) and restart nexelliad node to re-sync
				</p>
			</flow-form-control>
			<flow-form-control icon="fal:database">
				...
				<p slot="info" is="i18n-p">
					Start the nexelliad node without enabling the '--utxoindex' parameter until the synchronization is complete, re-enable this parameter. (Can be used to fix inaccurate balance display)
				</p>
			</flow-form-control>
			<div style="height:192px;"></div>
		</tab-content>
		<tab-content for="wallet" class="wallet" data-active-display="flex">
			<nexellia-wallet .walletMeta='${walletMeta}' hideNetwork hidefaucet 
				_hideQRScanner hideopenwalletlogo></nexellia-wallet>
		</tab-content>
...
//		this.checkForUpdates().then();
	...
	initRPC(){
		let rpc = new FlowRPC({bcastChannel:'nexellia-desktop'});
		this.rpc = rpc;
	...
	async initManager(){
		...
		manager.on("task-start", (daemon)=>{
			...
			this.refreshApps();
			const {wallet} = this;
			if(!wallet || daemon.task.type!='nexelliad' || !this.rpcDisconnect){
				return
			}
		...
		manager.on("before-interrupt", ({daemon, interrupt})=>{
			...
			const {wallet} = this;
			if(!wallet || daemon.task.type!='nexelliad')
				return
		...
		manager.on('sync-status', (data) => {
			//console.log("sync-status:data", data);
			let wallet = this.wallet || this.qS('nexellia-wallet');
	...
	async initWallet() {
		let wallet = this.qS('nexellia-wallet');
		this.wallet = wallet;
		this.applyCompoundConfig();
		let settings = await this.get_default_local_nexelliad_settings();
	...
	async get_default_local_nexelliad_settings() {
		
		let {config:daemons} = await this.get("get-modules-config");
		console.log("############### DAEMONS", daemons);
		let nexelliad = Object.entries(daemons).map(([k,v]) => { 
			const { args } = v;
			const [type, ident] = k.split(':');
			return { type, ident, args};
		}).filter(o=>o.type=='nexelliad').shift();

		if(!nexelliad)
			return null;//{network:"nexellia", port:33455};//{network:"nexelliatest", port:33755};

		const { args } = nexelliad;
		let networkType = ['testnet','devnet','simnet'].filter(v=>args[v] !== undefined).shift() || 'mainnet';
		let network = {
			mainnet : 'nexellia',
			testnet : 'nexelliatest',
			devnet : 'nexelliadev',
			simnet : 'nexelliasim'
		}[networkType];
		let { rpclisten } = args;
	...
	setTheme(theme){
		...
		if(this.caption)
			this.caption.logo = `/resources/images/nexellia-logo-${theme}-bg.png`
		...
		document.body.dispatchEvent(new CustomEvent("flow-theme-changed"));
		this.querySelector("nexellia-wallet")?.requestUpdate("theme", null)
	}
	initCaption(){
		...
		this.caption.logo = `/resources/images/nexellia-logo-${this.theme||"light"}-bg.png`;
	...
	async initSettings(){
		...
		if(statsdPrefixInput)
			this.statsdPrefix = statsdPrefixInput.value = config.statsdPrefix || "nexellia-desktop.$HOSTNAME";
...
${changelogContent}`;
	...
	async initDaemons(daemons){
		...
		let {params} = this.getModuleArgs('nexelliad:', daemons);
	...
	showApps(daemons) {
		...
		let entries = Object.values(appsMap).map((app) => {
			...
			if(!location && app.engines?.nexellia-desktop) {
			...
			return `${this.debugAppLinks?JSON.stringify(app)+"::"+location:""}
				<flow-window-link
					...
					width="${width}"
					height="${height}"
					icon="resources/images/nexellia-desktop-icon.png"
					resizable
					frame
...
NexelliaDesktopApp.define("nexellia-desktop-app")
