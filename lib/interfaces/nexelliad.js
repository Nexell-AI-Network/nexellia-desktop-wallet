...
const {Client:NexelliaGRPC} = require('@nexellia/grpc-node');
const pkg = require("../../package");
const natUpnp = require("@runonflux/nat-upnp");

module.exports = class Nexelliad extends Daemon {
	...
	start() {
		this.log("nexelliad::start() invoked...");
		return new Promise(async (resolve,reject) => {

			if(this.task.conf['reset-peers'])
				this.cleanupResidualData();

			this.log("nexelliad::start() running");
			...
			if (!("uacomment" in args)) {
				const uacomment = `nexellia-desktop_${pkg['version']}`
				this.writeToSink(`NEXELLIAD: Adding uacomment=${uacomment} to arguments\n`);
			...
			if (this.task.conf['upnpEnabled'] && args['listen'] && args['listen'].indexOf(":") !== -1) {
				...
				if (externalip) {
					if (!("externalip" in args)) {
						this.writeToSink(`NEXELLIAD: Adding externalip=${externalip} to arguments\n`);
					...
					if (publicPort) {
						...
						this.writeToSink(`NEXELLIAD: Changing externalip from ${args['externalip']} to ${externalip}:${publicPort} in arguments\n`);
						args['externalip'] = `${externalip}:${publicPort}`;
						if (publicPort !== nodePort) {
							this.writeToSink(`NEXELLIAD: Changing listener port from ${nodePort} to ${publicPort} in arguments\n`);
							args['listen'] = `${args['listen'].split(':')[0]}:${publicPort}`;
						}
						if (!("maxinpeers" in args)) {
							this.writeToSink(`NEXELLIAD: Adding maxinpeers=${this.upnpMaxInPeers} to arguments\n`);
			...
			const configFile = path.join(this.folder, 'nexelliad.conf');
			...
			await this.VSARGS([this.getDeployableBinary('nexelliad'),'--version']);

			this.run({
				args : async () => {
					let _args_ = [
 						this.getDeployableBinary('nexelliad')
					];
			...
			resolve();
			// mutes all nexelliad processes by default
			this.mute = true;
		})
	}
	...
	async stop() {
		...
		if(this.rpc) {
			...
			try {
				if(!process.env.NEXELLIA_JSON_RPC)				
		...
		return new Promise(async (resolve, reject) => {
	        ...
	        try {
				...
				this.data = this.sourcesToData(sources);
	             ...
	             console.log('NEXELLIAD monitor:'.bold,ex);
	            return reject(ex);
	        }
