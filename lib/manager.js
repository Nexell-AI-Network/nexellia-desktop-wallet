...
class NexelliaProcessManager extends EventEmitter{
	...
	async start(daemons={}) {
		...
		const startupTypeOrder = ['nexelliad','mqtt','mysql','pgsql', 'txgen','app'];
		const typeMap = { };
	...
	async stop() {
		//const shutdownTypeOrder = ['nexelliad','mqtt','mysql','pgsql','simulator','txgen'];
		//const typeMap = { };
	...
	getDeployableBinary(...args) {
		...
		switch(buildType) {
			...
			case 'local': {
				return path.join(os.homedir(),'.nexellia-desktop','bin',utils.platform, ...args)+this.PLATFORM_BINARY_EXTENSION;
			}			
		}
	}
	...
	enumerateApps() {
		...
		return packages.map(({ folder, pkg }) => {
			let { name, description, main, engines, nexellia } = pkg;
			return {
				folder, name, description, main, engines, ...nexellia
			}
		});
	}
...
module.exports = NexelliaProcessManager;
