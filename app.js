...
class App extends FlowApp{
	...
	*/
	dataDirInitError(){
		console.log(`Please start app with --init=/path/to/data/dir or --init for default (~/.nexellia-desktop/data)`);
		this.exit();
	}
	...
	*/
	...
	async removeDataDir(){
		try {
			let datadir2path = path.join(this.dataFolder, "nexelliad-kd0", "nexellia-mainnet", "datadir2");
	...
	*/
	...
	setSkipUTXOIndex(skipUTXOIndex){
		...
		return this.setModuleArgs("nexelliad:", {}, {"skip-utxoindex":skipUTXOIndex})
	}
	getSkipUTXOIndex(){
		let {args, params} = this.getModuleArgs("nexelliad:");
	...
	setUpnpIfMissing(upnpEnabled) {
		if (this.config?.modules) {
			Object.keys(this.config.modules).forEach((k) =>{
				if (k.startsWith('nexelliad:') && !('upnpEnabled' in this.config.modules[k])) {
